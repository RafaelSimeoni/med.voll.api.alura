...
public class SecurityConfigurations {
    ...
    public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
        return http.csrf().disable() //desabilita a proteção contra csrf (o token já é uma proteção pra isso)
                ...
                .requestMatchers(HttpMethod.POST, "/login").permitAll() //liberar o endpoint de login
                .anyRequest().authenticated() //Para todos os outros é necessário estar autenticado
                .and().addFilterBefore(securityFilter, UsernamePasswordAuthenticationFilter.class) //serve para chamar o nosso filtro criado antes do filtro do spring
                .build();
    }
